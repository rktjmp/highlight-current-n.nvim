...
local function highlight_current(buf, pos_row, pos_col)
  ...
  local ext_mark_opts = {virt_text = {{matched_text, highlight_group_name()}}, virt_text_pos = "overlay", end_line = (pos_row - 1), end_col = (pos_col + #matched_text)}
  ...
  return vim.cmd(augroup_cmds)
end
local function feedkey(key)
  local win = vim.api.nvim_get_current_win()
  local buf = vim.api.nvim_get_current_buf()
  local before_err = vim.v.errmsg
  local maybe_highlight
  local function _1_()
    if (before_err == vim.v.errmsg) then
      local _let_2_ = vim.api.nvim_win_get_cursor(win)
      local row = _let_2_[1]
      local col = _let_2_[2]
  ...
  return vim.schedule(maybe_highlight)
end
local function searched()
  local win = vim.api.nvim_get_current_win()
  local buf = vim.api.nvim_get_current_buf()
  local _let_4_ = vim.api.nvim_win_get_cursor(win)
  local before_row = _let_4_[1]
  local before_col = _let_4_[2]
  local maybe_highlight
  local function _5_()
    local _let_6_ = vim.api.nvim_win_get_cursor(win)
    local after_row = _let_6_[1]
    local after_col = _let_6_[2]
...
return {n = _8_, N = _9_, ["/,?"] = _10_, setup = setup}
